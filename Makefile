...
build-ext: l10n-compile  ## Build Sphinx extension.
	npm run build
...
l10n-extract:  ##  Extract l10n messages to sphinx_nefertiti/locale/sphinx.pot.
...
l10n-create:  ## Add new locale (pass argument LOCALE=<iso-639-1-code>).
	@pybabel init \
		-i sphinx_nefertiti/locale/sphinx.pot \
		-d sphinx_nefertiti/locale \
		-l ${LOCALE} \
...
l10n-update:  ## Update existing locales with new extracted messages.
	@pybabel update \
		-i sphinx_nefertiti/locale/sphinx.pot \
		-d sphinx_nefertiti/locale \
		-D sphinx

l10n-compile:  ## Compile all locale PO files into binary MO files.
