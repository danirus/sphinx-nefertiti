export function resizeAsides() {
  ...
  const nftt_page = document.querySelector(".nftt-page");
  const nftt_content = document.querySelector(".nftt-content");
  const nftt_content_child = document.querySelector(".nftt-content > div");
  ...
  if (nftt_content_child == undefined)
    return "";

  if (nftt_content_child.clientHeight < document.body.clientHeight) {
    // The content of the document is shorter than the browser height.
    ...
    height = h < nftt_content_child.clientHeight
      ? `height: ${nftt_content_child.clientHeight}px`
      : `height: ${h}px`;
  } else {
  ...
  nftt_page.setAttribute("style", `height: ${nftt_content.clientHeight}px`);
